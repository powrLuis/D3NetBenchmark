...
from dataloader_rgbdsod import RgbdToDataset
...
test_loaders=[]
for test_dataset in test_datasets:
    val_set=RgbdToDataset(datasets_path+test_dataset,transform=transform_test)
...
for index, test_loader in enumerate(test_loaders):
    ...
    for i, sample_batched in enumerate(tqdm(test_loader)):
        input = model_rgb.get_input(sample_batched)
        ...
        gt_src=np.array(Image.open(sample_batched['meta']['depth_path'][0]).convert('L'))
